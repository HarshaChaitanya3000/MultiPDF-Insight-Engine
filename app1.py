...
# *** KEY CHANGE: Load credentials from Streamlit secrets ***
try:
    service_account_info = json.loads(st.secrets["GOOGLE_SERVICE_ACCOUNT_JSON"])
    credentials = google.auth.credentials.Credentials.from_service_account_info(service_account_info) # Corrected import
    genai.configure(credentials=credentials)

except (json.JSONDecodeError, KeyError) as e:  # Catch JSON or secret errors
    st.error(f"Error loading service account key: {e}")
    st.stop()

# --- (Rest of your functions: get_pdf_text, get_text_chunks, get_vector_store, 
#      get_conversational_chain, and user_input remain UNCHANGED) ---
