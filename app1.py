...
# Load environment variables (optional if you're using .env for local settings)
load_dotenv()

# Access the Google service account credentials from Streamlit secrets
google_service_account_json = st.secrets["google"]["google_service_account_json"]

# Parse the Google service account credentials from the secret (string format)
google_credentials_info = json.loads(google_service_account_json)

# Save the credentials to a temporary file (required for Google authentication)
with open("temp_credentials.json", "w") as f:
    json.dump(google_credentials_info, f)
